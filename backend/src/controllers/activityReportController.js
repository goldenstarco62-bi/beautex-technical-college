...
export const getConsolidatedReport = async (req, res) => {
    try {
        ...
        if (mongo) {
            ...
            [reports, trainerReports] = await Promise.all([
                ActivityReport.find({
                    report_type: 'daily',
            ...
            [reports, trainerReports] = await Promise.all([
                ...
                query(
                    'SELECT * FROM trainer_reports WHERE report_date BETWEEN ? AND ?',
                    [startDate, endDate]
                )
            ]);
        }

        if (reports.length === 0 && trainerReports.length === 0) {
        ...
        const consolidated = {
            ...
            total_trainer_reports: trainerReports.length,
            stats: {
        ...
        reports.forEach(report => {
            ...
            consolidated.stats.total_students_expected += (report.total_students_expected || 0);
            ...
            consolidated.stats.classes_conducted += (report.classes_conducted || 0);

            if (!consolidated.departmental_breakdown[dept]) {
                consolidated.departmental_breakdown[dept] = { count: 0 };
            }
            ...
            // Extended fields
            if (report.facilities_issues) consolidated.qualitative.facilities_issues.push(prefix + report.facilities_issues);
            if (report.equipment_maintenance) consolidated.qualitative.equipment_maintenance.push(prefix + report.equipment_maintenance);
            if (report.cleaning_maintenance) consolidated.qualitative.cleaning_maintenance.push(prefix + report.cleaning_maintenance);
            if (report.internet_ict_status) consolidated.qualitative.internet_ict_status.push(prefix + report.internet_ict_status);
            if (report.student_feedback) consolidated.qualitative.student_feedback.push(prefix + report.student_feedback);
            if (report.counseling_support) consolidated.qualitative.counseling_support.push(prefix + report.counseling_support);
            if (report.social_media_activities) consolidated.qualitative.social_media_activities.push(prefix + report.social_media_activities);
            if (report.actions_taken) consolidated.qualitative.actions_taken.push(prefix + report.actions_taken);
        });

        // Process Trainer Reports
        ...
        trainerReports.forEach(tr => {
            ...
            if (tr.daily_report) consolidated.qualitative.topics_covered.push(`[Trainer ${tr.trainer_name}] ${tr.daily_report}`);
            if (tr.record_of_work) consolidated.qualitative.practical_sessions.push(`[Trainer ${tr.trainer_name}] ${tr.record_of_work}`);
        });
...
/**
 * NEW: Consolidated Departmental Report Assistant Logic
 * Groups qualitative data strictly by department as requested.
 */
function generateAINarrative(deptBreakdown, stats, startDate, endDate, warnings) {
    const depts = Object.keys(deptBreakdown).sort();
    const activeDepts = depts.filter(d => deptBreakdown[d].classCount > 0 || deptBreakdown[d].trainerReportCount > 0 || deptBreakdown[d].academicReportCount > 0);
    const quietDepts = warnings.filter(w => w.type === 'no_submissions').map(w => w.department);

    const dateLabel = startDate === endDate
        ? `on ${new Date(startDate).toLocaleDateString('en-US', { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' })}`
        : `from ${new Date(startDate).toLocaleDateString('en-US', { day: 'numeric', month: 'short', year: 'numeric' })} to ${new Date(endDate).toLocaleDateString('en-US', { day: 'numeric', month: 'short', year: 'numeric' })}`;

    let narrative = `Academic operations ${dateLabel} involved ${stats.totalReports + stats.totalTrainerReports + stats.totalAcademicReports} consolidated reports across ${activeDepts.length} active department${activeDepts.length !== 1 ? 's' : ''}. `;

    // Per-department highlights
    activeDepts.slice(0, 5).forEach((dept, i) => {
        const d = deptBreakdown[dept];
        const topicsSnippet = (d.topics || []).filter(Boolean).slice(0, 2).join(', ');
        
        if (i === 0) {
            narrative += `The ${dept} department recorded ${d.classCount} classes and ${d.trainerReportCount + d.academicReportCount} specialized reports`;
        } else if (i === activeDepts.length - 1 && activeDepts.length > 1) {
            narrative += `, while ${dept} managed ${d.classCount} sessions`;
        } else {
            narrative += `; the ${dept} department covered ${d.classCount} classes`;
        }

        if (topicsSnippet) narrative += ` focusing on ${topicsSnippet}`;
    });

    narrative += '. ';

    // Attendance summary
    const attendRate = stats.totalStudentsExpected > 0
        ? Math.round((stats.totalStudentsPresent / stats.totalStudentsExpected) * 100)
        : null;

    if (attendRate !== null) {
        if (attendRate >= 80) {
            narrative += `Student attendance was strong at ${attendRate}% (${stats.totalStudentsPresent} present), reflecting high engagement. `;
        } else if (attendRate >= 60) {
            narrative += `Student attendance stood at ${attendRate}% (${stats.totalStudentsPresent} of ${stats.totalStudentsExpected}), indicating moderate participation. `;
        } else {
            narrative += `Student attendance was below target at ${attendRate}% (${stats.totalStudentsPresent} present), warranting follow-up. `;
        }
    }

    // Challenges
    const allChallenges = activeDepts.flatMap(d => deptBreakdown[d].challenges || []).filter(Boolean);
    if (allChallenges.length > 0) {
        const challengeSnippet = allChallenges.slice(0, 2).join('; ');
        narrative += `Key challenges reported include: ${challengeSnippet}. `;
    }

    if (quietDepts.length > 0) {
        narrative += `Note: No reports were submitted for the ${quietDepts.join(', ')} department${quietDepts.length !== 1 ? 's' : ''} during this period. Follow-up is recommended.`;
    } else {
        narrative += 'All tracked departments submitted reports for the period.';
    }

    return narrative;
}

export const getDepartmentalConsolidatedReport = async (req, res) => {
    try {
        const { startDate, endDate } = req.query;
        if (!startDate || !endDate) {
            return res.status(400).json({ success: false, error: 'Dates are required' });
        }
        ...
        if (mongo) {
            ...
            for (const tr of trainerReports) {
                if (tr.course_id) {
                    const course = await Course.findOne({ id: tr.course_id }).lean();
                    if (course) tr.department = course.department;
                }
            }
        } else {
            [reports, trainerReports] = await Promise.all([
        ...
        if (reports.length === 0 && trainerReports.length === 0) {
            return res.json({ success: true, report: 'No data available for the selected period.' });
        }

        const departments = {};
        let totalStats = { present: 0, expected: 0, enrollments: 0 };

        reports.forEach(r => {
            const dept = r.department || 'General';
            if (!departments[dept]) departments[dept] = [];
            
            totalStats.present += (parseInt(r.total_students_present) || 0);
            totalStats.expected += (parseInt(r.total_students_expected) || 0);
            totalStats.enrollments += (parseInt(r.new_enrollments) || 0);

            const entry = [
                r.topics_covered ? `Topics: ${r.topics_covered}` : '',
                r.practical_sessions ? `Practicals: ${r.practical_sessions}` : '',
                r.challenges_faced ? `Challenges: ${r.challenges_faced}` : ''
            ].filter(Boolean).join('. ');

            if (entry) departments[dept].push(`[General] ${entry}`);
        });

        trainerReports.forEach(tr => {
            const dept = tr.department || 'Academic';
            if (!departments[dept]) departments[dept] = [];

            const entry = `Trainer ${tr.trainer_name}: ${tr.daily_report}. Record of Work: ${tr.record_of_work}`;
            departments[dept].push(`[Trainer] ${entry}`);
        });

        let reportLines = ['# Consolidated Department Report\n'];
        
        Object.keys(departments).sort().forEach(dept => {
            if (departments[dept].length === 0) return;
            reportLines.push(`## ${dept}`);
            const uniqueEntries = [...new Set(departments[dept])];
            uniqueEntries.forEach(e => reportLines.push(`- ${e}`));
            reportLines.push('');
        });

        const avgAttend = totalStats.expected > 0 ? Math.round((totalStats.present / totalStats.expected) * 100) : 0;
        
        reportLines.push('## Overall Summary');
        reportLines.push(`Over the reporting period, ${reports.length} general logs and ${trainerReports.length} trainer academic reports were consolidated. Average student attendance: ${avgAttend}%. New enrollments: ${totalStats.enrollments}. Training activities across departments were successfully captured, showing consistent curriculum delivery by the faculty.`);
...
export const getAcademicSummary = async (req, res) => {
    try {
        const { startDate, endDate, department, trainer } = req.query;
        ...
        if (mongo) {
            ...
            if (department) actFilter.department = department;

            const trFilter = {
                report_date: { $gte: start, $lte: end }
            };
            if (trainer) trFilter.trainer_name = { $regex: trainer, $options: 'i' };
            ...
            [activityReports, trainerReports, academicReportsData] = await Promise.all([
                ActivityReport.find(actFilter).sort({ report_date: 1 }).lean(),
                TrainerReport.find(trFilter).sort({ report_date: 1 }).lean(),
                AcademicReport.find(acadFilter).sort({ report_date: 1 }).lean()
            ]);
        } else {
            let actSql = 'SELECT * FROM daily_activity_reports WHERE (DATE(report_date) BETWEEN ? AND ?)';
            let actParams = [startDate, endDate];
            if (department) { actSql += ' AND department = ?'; actParams.push(department); }

            let trSql = `SELECT tr.*, 
                                COALESCE(c1.department, c2.department) as course_department
                         FROM trainer_reports tr
                         LEFT JOIN courses c1 ON CAST(tr.course_id AS TEXT) = CAST(c1.id AS TEXT)
                         LEFT JOIN courses c2 ON LOWER(TRIM(CAST(tr.course_id AS TEXT))) = LOWER(TRIM(c2.name))
                         WHERE (DATE(tr.report_date) BETWEEN ? AND ?)`;
            let trParams = [startDate, endDate];
            if (trainer) { trSql += ' AND tr.trainer_name LIKE ?'; trParams.push("%" + trainer + "%"); }

            let acadSql = `SELECT ar.*, 
                                 COALESCE(c1.department, c2.department) as course_department
                           FROM academic_reports ar
                           LEFT JOIN courses c1 ON LOWER(TRIM(ar.course_unit)) = LOWER(TRIM(c1.name))
                           LEFT JOIN courses c2 ON CAST(ar.course_unit AS TEXT) = CAST(c2.id AS TEXT)
                           WHERE (DATE(ar.created_at) BETWEEN ? AND ?)`;
            let acadParams = [startDate, endDate];

            const [_act, _tr, _acad] = await Promise.allSettled([
                query(actSql + " ORDER BY report_date ASC", actParams),
                query(trSql + " ORDER BY tr.report_date ASC", trParams),
                query(acadSql + " ORDER BY ar.created_at ASC", acadParams)
            ]);
            if (_act.status  === "fulfilled") activityReports     = _act.value;
            if (_tr.status   === "fulfilled") trainerReports      = _tr.value;
            if (_acad.status === "fulfilled") academicReportsData = _acad.value;
        }

        let systemDepts = [];
        if (mongo) {
            const Course = (await import('../models/mongo/Course.js')).default;
            systemDepts = await Course.distinct('department');
        } else {
            const courses = await query('SELECT DISTINCT department FROM courses');
            systemDepts = courses.map(c => c.department).filter(Boolean);
        }

        const allKnownDepts = [...new Set([
            ...systemDepts,
            ...activityReports.map(r => r.department || 'General'),
            ...trainerReports.map(r => r.department || r.course_department || 'Academic'),
            ...academicReportsData.map(r => r.course_department || 'Academic')
        ])].filter(d => d !== 'General' && d !== 'Academic').sort();

        if (activityReports.some(r => !r.department || r.department === 'General')) {
            allKnownDepts.push('General');
        }
        if (trainerReports.some(tr => !tr.department && !tr.course_department) || academicReportsData.some(r => !r.course_department)) {
            allKnownDepts.push('Academic');
        }

        const departmentBreakdown = {};
        allKnownDepts.forEach(dept => {
            departmentBreakdown[dept] = {
                ...
                challenges: [],
                achievements: [],
                trainerReportCount: 0,
                academicReportCount: 0
            };
        });
        ...
        activityReports.forEach(r => {
            ...
            if (!departmentBreakdown[dept]) {
                departmentBreakdown[dept] = { classCount: 0, studentsPresent: 0, studentsExpected: 0, topics: [], practicals: [], challenges: [], achievements: [], trainerReportCount: 0, academicReportCount: 0 };
            }
            departmentBreakdown[dept].classCount   += (parseInt(r.classes_conducted) || 1);
            departmentBreakdown[dept].studentsPresent  += (parseInt(r.total_students_present) || 0);
            departmentBreakdown[dept].studentsExpected += (parseInt(r.total_students_expected) || 0);

            if (r.topics_covered)    departmentBreakdown[dept].topics.push(r.topics_covered);
            if (r.practical_sessions) departmentBreakdown[dept].practicals.push(r.practical_sessions);
            if (r.challenges_faced)  departmentBreakdown[dept].challenges.push(r.challenges_faced);
            if (r.achievements)      departmentBreakdown[dept].achievements.push(r.achievements);
        });

        trainerReports.forEach(tr => {
            const dept = tr.department || tr.course_department || 'Academic';
            if (!departmentBreakdown[dept]) {
                departmentBreakdown[dept] = { classCount: 0, studentsPresent: 0, studentsExpected: 0, topics: [], practicals: [], challenges: [], achievements: [], trainerReportCount: 0, academicReportCount: 0 };
            }
            departmentBreakdown[dept].trainerReportCount += 1;
            if (tr.daily_report)    departmentBreakdown[dept].topics.push(tr.daily_report);
            if (tr.record_of_work)  departmentBreakdown[dept].practicals.push(tr.record_of_work);
        });

        academicReportsData.forEach(ar => {
            const dept = ar.course_department || 'Academic';
            if (!departmentBreakdown[dept]) {
                departmentBreakdown[dept] = { classCount: 0, studentsPresent: 0, studentsExpected: 0, topics: [], practicals: [], challenges: [], achievements: [], trainerReportCount: 0, academicReportCount: 0 };
            }
            departmentBreakdown[dept].academicReportCount += 1;
            if (ar.theory_topics)    departmentBreakdown[dept].topics.push(`Theory: ${ar.theory_topics}`);
            if (ar.practical_tasks) departmentBreakdown[dept].practicals.push(`Task: ${ar.practical_tasks}`);
            if (ar.progress_summary) departmentBreakdown[dept].achievements.push(`Progress: ${ar.progress_summary}`);
        });

        const stats = {
            totalReports: activityReports.length,
            totalTrainerReports: trainerReports.length,
            totalAcademicReports: academicReportsData.length,
            totalStudentsPresent: Object.values(departmentBreakdown).reduce((s, d) => s + d.studentsPresent, 0),
            totalStudentsExpected: Object.values(departmentBreakdown).reduce((s, d) => s + d.studentsExpected, 0),
            totalClassesConducted: Object.values(departmentBreakdown).reduce((s, d) => s + d.classCount, 0),
            activeDepartments: Object.values(departmentBreakdown).filter(d => d.classCount > 0 || d.trainerReportCount > 0 || d.academicReportCount > 0).length,
            totalDepartments: allKnownDepts.length
        };
        ...
        activityReports.forEach(r => {
            ...
            if (r.report_date) {
                try {
                    dateStr = typeof r.report_date === "string" 
                        ? r.report_date.split("T")[0] 
                        : new Date(r.report_date).toISOString().split("T")[0];
                } catch (e) {
                    console.error("Invalid date encountered:", r.report_date);
                    return;
                }
            }
            if (!dateStr) return;
            if (!trendMap[dateStr]) trendMap[dateStr] = { date: dateStr, present: 0, expected: 0, classes: 0 };
            trendMap[dateStr].present  += (parseInt(r.total_students_present) || 0);
            trendMap[dateStr].expected += (parseInt(r.total_students_expected) || 0);
            trendMap[dateStr].classes  += (parseInt(r.classes_conducted) || 1);
        });
        const attendanceTrend = Object.values(trendMap).sort((a, b) => a.date.localeCompare(b.date));

        const departmentActivity = Object.entries(departmentBreakdown).map(([dept, d]) => ({
            department: dept,
            classes: d.classCount,
            students: d.studentsPresent,
            trainerReports: d.trainerReportCount
        })).sort((a, b) => b.classes - a.classes);

        const warnings = [];
        allKnownDepts.forEach(dept => {
            const d = departmentBreakdown[dept];
            if (d.classCount === 0 && d.trainerReportCount === 0) {
                warnings.push({ type: 'no_submissions', department: dept, message: `No reports submitted for ${dept}` });
            } else if (d.studentsExpected > 0 && d.studentsPresent / d.studentsExpected < 0.5) {
                warnings.push({ type: 'low_attendance', department: dept, message: `Low attendance in ${dept}: ${Math.round((d.studentsPresent / d.studentsExpected) * 100)}%` });
            }
        });

        const aiNarrative = generateAINarrative(departmentBreakdown, stats, startDate, endDate, warnings);
        ...
        res.json({
            ...
            data: {
                ...
                filters: { department: department || null, trainer: trainer || null }
            }
        });
